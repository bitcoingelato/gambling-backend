...
function authenticateToken(req, res, next) {
    ...
    if (!auth) return res.json({ success: false, message: "No token" });
...
// ====== Crash Game (simple: one global round) ======
let crashState = {
    roundId: 1,
...
function startCrashRound() {
    ...
    crashState.bets = {};
    // Simulate crash point (1.01 to 5x, weighted low)
    ...
    let interval = setInterval(async () => {
        ...
        if (crashState.multiplier >= crashState.crashAt) {
            ...
            // Resolve bets
            for (const u in crashState.bets) {
                if (!crashState.bets[u].cashedOut) {
                    // lose
                }
            }
            await crashCollection.updateOne({ roundId: crashState.roundId }, { $set: { crashAt: crashState.crashAt, ended: new Date(), bets: Object.values(crashState.bets) } });
...
app.post('/api/coinflip/bet', authenticateToken, async (req, res) => {
    ...
    let payout = 0;
    if (win) payout = amount;
    await usersCollection.updateOne({ username: user.username }, { $inc: { balance: payout - amount } });
    await coinflipCollection.insertOne({ username: user.username, amount, choice, result, win, created: new Date() });
    res.json({ success: true, result, win, payout: win ? amount : 0 });
});
...
// ====== 3 Card Poker (very simple example) ======
const deck = () => {
    const suits = ['H','D','C','S'];
...
app.post('/api/3cp/bet', authenticateToken, async (req, res) => {
    ...
    // Simple: player wins if their highest card is higher
